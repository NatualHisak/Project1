...
from Functions import updateSchedule
    ...
    if 'user' not in st.session_state and 'password' not in st.session_state:
        ...
        with st.form(key='form_input'):
           ...
           'Hose Check Report Platform'),index = 0)
            ...
            tipe = st.selectbox('Weekly', 'Monthly', 'Tri-Monthly', 'Six-Monthly', 'Yearly')
            base = st.selectbox('terminal 1', 'terminal 2', 'terminal 3')
            ...
            if submitted:
                #tambahin cause, follup, sparepart, status di database
                updateSchedule(activity, SKH, tipe, sch_date, base, fin_date)